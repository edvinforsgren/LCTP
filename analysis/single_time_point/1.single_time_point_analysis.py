...
def process_compound(cmp_idx, excl_plate, global_seed, df, moas, compound_list, hour, epochs, layers, batch_size, learning_rate, activation_function, loss_function, drop_out):
    ...
    # Prepare test data
    X_test, y_encoded_test, test_df = ut.prepare_data_mod(df, [excl_plate], hours, moas, train=False, 
                                                        excl_metadatas=['Metadata_Plate'])
    ...
    pred_labels.append([f'{c}_pred' for c in moas])
...
def run_all_data(df, moas, compound_list, hour, epochs, layers, batch_size, learning_rate, activation_function, loss_function, drop_out, global_seed):
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        results = pool.starmap(process_compound,
            [(plate_idx, unique_plates[plate_idx], global_seed, df, moas, compound_list, hour, epochs, layers, batch_size, learning_rate, activation_function, loss_function, drop_out)
...
def calculate_max_moa_endpoint(row, moas):
    """Calculate the maximum MOA prediction for a row"""
    sums = []
    for moa in moas:
        cols = [c for c in row.index if (c.startswith(moa) and c.endswith("pred"))]
        sums.append(row[cols].sum())
    return moas[np.argmax(sums)]
...
if __name__  == "__main__":
    ...
    df_final['max_sum'] = df_final.apply(lambda row: calculate_max_moa_endpoint(row, moas), axis=1)
    
    # Save results
    df_final.to_parquet(f'{save_dir}/{file_name}_cmpd_nn_eq_pred_processed.parquet.gzip', compression='gzip')
    full_dmsos.to_parquet(f'{save_dir}/{file_name}_dmso_nn_eq_pred_processed.parquet.gzip', compression='gzip')
    # Write paths to config.yaml file in comparison directory
    ...
    if 'single_timepoint' not in config:
        config['single_timepoint'] = {}

    # Add your new key-value pairs under 'single_timepoint'
    config['single_timepoint']['single_time_point_analysis'] = f"{save_dir}/{file_name}_cmpd_nn_eq_pred.parquet.gzip"
    config['single_timepoint']['single_time_point_analysis_dmso'] = f"{save_dir}/{file_name}_dmso_nn_eq_pred.parquet.gzip"
    config['single_timepoint']['single_time_point_analysis_processed'] = f"{save_dir}/{file_name}_cmpd_nn_eq_pred_processed.parquet.gzip"
    config['single_timepoint']['single_time_point_analysis_dmso_processed'] = f"{save_dir}/{file_name}_dmso_nn_eq_pred_processed.parquet.gzip"
